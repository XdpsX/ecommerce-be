...
public interface CategoryRepository extends JpaRepository<Category, Integer>, JpaSpecificationExecutor<Category> {
    ...
    /**
     * Row-locked lookup used by every hierarchy mutation. Locking the node itself
     * serializes two moves of the same
     * category before any parent or order is read.
     *
     * <p>
     * {@code LEFT JOIN FETCH} cannot be combined with a lock query, so only the
     * node is fetched and the parent is
    ...
    /**
     * Ordered sibling group of a non-root parent. The sort matches the
     * {@code (parent_id, display_order)} index so
    ...
    /**
     * Ordered root sibling group ({@code parent_id IS NULL}), locked for hierarchy
     * mutations.
     */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    ...
    /**
     * Row-locked {@code id -> parentId} projection of one node, used to walk (and
     * lock) the ancestor chain.
     *
     * <p>
     * A projection rather than {@code SELECT c.parent} because a root row has a
     * {@code null} parent and would
     * otherwise be indistinguishable from a missing row. Locking each ancestor is
     * what makes the computed level stable
    ...
    /**
     * {@code id -> parentId} projection of every category, used to compute a
     * subtree height without mapping a
     * {@code children} collection. The taxonomy is intentionally small, so one flat
     * projection is cheaper than a
     ...
     *
     * <p>
     * This row is what makes hierarchy writes serialize. It exists because a
     * sibling group cannot always be locked
     * through its own rows: a root group is addressed by {@code parent_id IS NULL},
     * and an empty group has no row at
     * all. Locking one shared row gives every writer something to contend on.
     *
     * <p>
     * The lock is a plain InnoDB row lock, so it is released exactly when the
     * transaction commits or rolls back. That
     * is why this is used instead of {@code GET_LOCK}: a MySQL named lock is bound
     * to the physical session and would
    ...
    /**
     * Whether the anchor row exists. A dedicated query is needed because
     * {@code existsById} would check the
     * {@code categories} table, and the anchor shares the id space by accident
     * rather than by relation.
     */
    @Query("SELECT COUNT(a) > 0 FROM CategoryHierarchyAnchor a WHERE a.id = 1")
    boolean hierarchyAnchorExists();

    /**
     * Storefront flat read model: every category with its image and the whole
     * ancestor chain fetched in one
     * statement, ordered for tree assembly. Effective status is derived in memory
     * from the fetched chain
     * ({@link Category#isEffectivelyActive()}), so this query intentionally does
     * not filter on status.
     *
     * <p>
     * The taxonomy is bounded at {@link Category#MAX_DEPTH} levels, so fetching the
     * parent chain twice is
    ...
    /**
     * Batch lookup with the full ancestor chain, used to validate a whole requested
    ...
    /**
     * Storefront lookup. Named for the stored status it filters on; effective
     * status across the ancestor chain
     * is derived via {@link #findByIdWithAncestry(Integer)} instead.
     */
